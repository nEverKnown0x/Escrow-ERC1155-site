...
export default class BlockChainInterface {
  ...
  async removeEscrow(addressContract, index) {
    const addressAccount = (await this.web3Obj.eth.getAccounts())[0];
    const contract = new this.web3Obj.eth.Contract(ABI_ESCROW, addressContract);

    let result = await contract.methods.removeEscrow(index).send({from: addressAccount});
    console.log(await result);
  }

  async executeEscrow(addressContract, index) {
    const addressAccount = (await this.web3Obj.eth.getAccounts())[0];
    const contract = new this.web3Obj.eth.Contract(ABI_ESCROW, addressContract);

    let result = await contract.methods.executeEscrow(index).send({from: addressAccount});
    console.log(await result);
  }

  async getEscrow(addressContract, index) {
    const addressAccount = (await this.web3Obj.eth.getAccounts())[0];
    const contract = new this.web3Obj.eth.Contract(ABI_ESCROW, addressContract);

    let result = await contract.methods.getEscrow(index).call({from: addressAccount});
    console.log(await result);
    ...
    let resultViewable = "nftOwner: " + resultParsed[0]
      ...
      + "\nnftID: " + resultParsed[2]
      + "\nnftAmount: " + resultParsed[3]
      + "\ntoken to recieve: " + resultParsed[4]
      + "\ntokens: " + resultParsed[5] / 10**18;
    alert(resultViewable);
    return resultStruct;
  }

  async getMyEscrowIndexes(addressContract) {
    const addressAccount = (await this.web3Obj.eth.getAccounts())[0];
    const contract = new this.web3Obj.eth.Contract(ABI_ESCROW, addressContract);

    let result = await contract.methods.getMyEscrowIndexes().call({from: addressAccount});
    console.log(await result);
    return result;
  }
